...
> 🚨 **UI를 만들기 전에 [14 UI 작업 대전제](14-ui-ground-rules.md)를 읽을 것** —
> 언어는 한국어·일본어 둘, **글자가 든 그림은 통째로 언어별 교체**, 폰트는 상태·수치 표시.
>
...
`go_router` redirect로 **인증/온보딩 게이트 하나**: 토큰 없음 → 로그인 / 프로필 미완성 → 온보딩.

> 🗑️ **시간 게이트(`system/gate`)는 폐지됐다**(Plan_3 ①단계). 24시간 운영이라 대기 화면이 없다.
> 대신 **영업일 경계 KST 18시**가 남았는데, 이건 화면을 막지 않고 "오늘 것"의 범위만 정한다.
