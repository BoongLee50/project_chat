...
> **싱크 요약 ③ — 오류 다국어화 + 번역 BM 구멍 차단** (`048c1a5` → `23f3195`)
> 사용자에게 **보이는 것**(오류 문구)과 **돈을 낸 것**(번역 패스), 둘 다 말만 있고 실체가 없던 구간을 메웠다.
>
> | # | 한 일 | 서버 | 커밋 |
> |---|---|---|---|
> | 15 | **서버 `ErrorCode` 세분화** — `VALIDATION_FAILED` 하나에 36곳이 뭉쳐 클라가 분기할 수 없었다. 62개 호출부에 `도메인_상황` 코드 부여 | 변경 | `048c1a5` |
> | 16 | **클라 오류 매핑 배관** — 프로바이더 7개 `Future<String?>`→`Future<ApiException?>`, 화면 25곳 | — | `2252841` |
> | 17 | **오류 문구 한·일 이전** — ARB `error*` 61키(양쪽 439키), 클라 자체 오류 5종 포함 | — | `2b14f6a` |
> | 18 | **서버 enum ↔ 클라 매핑 정합성 테스트** | — | `ca37824` |
> | 19 | **번역 무료 쿼터·패스 판정** — 댓글 2회/채팅 2명/프로필 무료, 패스·프라임 무제한 | **V7** | `23f3195` |
>
> **이 구간에서 굳힌 원칙**
