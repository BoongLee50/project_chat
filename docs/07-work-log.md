...
**한 줄 요약**: 기획서 화면 1~30 전부 구현. **전 화면이 로컬 서버·DB와 실제 연동 완료** — 로그인/온보딩 · 프로필 · 홈(오늘의 포스트) · 달빛가든 · 대화방/채팅창(WebSocket 실시간) · 친구. **서버 도메인 전부**(Flyway **V8**) 구현 완료. **UI 다국어(한↔일) ①·②단계 완료** — 화면 전량 + **오류 문구까지** ARB 이전(459키). **번역 무료 쿼터·패스 판정**도 붙어 `TRANSLATE_PASS`가 실제 혜택을 갖는다. **음성 메시지**(녹음·전송·재생, V9)는 두 기기로 실시간 송수신까지 확인됨. **달빛가든 시안(Plan_3) 적용 완료**이고 클라 갱신은 "낡았으면 1분" 방식으로 통일했다. 🚀 **Plan_3(v2.0) 반영 ①②단계 완료**(2026-08-29, **V10·V11**) — **야간 게이트가 폐지돼 24시간 열려 있고**, "하루"의 경계가 **KST 18시**다. 하루 한 마디·포스트 등록 창·[보낸 신청] 탭·스포트라이트 부스트도 함께 걷어냈다. ②단계로 **포스트 메인 사진**(달빛가든 대표 사진)을 붙였다 — 최초 사진 자동 메인·삭제 시 이웃 슬롯 승계·최신이 1번 슬롯.
**다음은 ③단계(달빛가든 — 열람 제한·노출 알고리즘)**([09](09-next-task-handoff.md) · 분석 [12](12-plan3-diff.md)). 일본어 마감은 **보류**(화면이 바뀌면 문구도 바뀐다). 그 외 남은 것: 번역 공급자 연동(API 키) · 앱 아이콘·스플래시 · 외부 계정이 필요한 것들 · **피드 확장성 부채 2건**(Plan_3 ③단계에서 함께 해소된다).
...
| DB | 로컬 MariaDB 11.4.5, Flyway **V11까지 적용**(V1 초기 · V2 posts 등록창/교체 · V3 post_comments · V4 chat · V5 friendships 양방향 · V6 BM · V7 번역 쿼터 · V8 스킵 복귀/갱신 감지 · V9 음성 메시지 · V10 게이트 폐지 정리 · **V11 메인 사진**) |
...
| 다국어 | **①·②단계 완료** — `app_ko.arb`/`app_ja.arb` 각 **432키**(①단계에서 30키가 사라지고 ②단계에서 메인 사진 3키가 늘었다). 기기 언어를 따르고 미지원 언어는 한국어 폴백. 오류 문구는 서버 `ErrorCode`(56종)를 클라가 번역한다. 에뮬 `ja-JP` 실확인. 남은 건 **원어민 검수**(③단계, 출시 블로커) |
