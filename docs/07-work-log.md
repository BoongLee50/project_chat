...
**한 줄 요약**: 화면 11개 완성. **전 화면이 로컬 서버·DB와 실제 연동 완료** — 로그인/온보딩 · 프로필 · 홈(오늘의 포스트) · 달빛가든 · 대화방/채팅창(WebSocket 실시간) · 친구. **서버 도메인은 전부 구현 완료**(Flyway V6). 남은 건 **BM 화면 6종**과 세부 팝업, 그리고 외부 계정이 필요한 것들.
...
**남은 화면/기능 한눈에**: **BM 화면 6종(25~30)** · 신고/차단 팝업 · 관심사/지역/소개 편집 팝업 · 친구 오늘의 포스트 팝업 · 시간 게이트 화면 분기 · 다국어(한↔일).

서버는 사실상 다 됐고, **남은 건 클라 화면과 외부 계정이 있어야 하는 것들**(영수증 실검증·웹훅 서명·광고·푸시·S3)이다.
...
1. **BM 화면 6종 신규(25~30)** — 루나상점 · 프라임 멤버십 · 루나 충전샵 · 포스트 부스트 · 앨범 패스 · 자동 번역 패스.
   서버는 준비 끝났고 **`GET /store/products`(구성) + `GET /me/wallet`(내 상태)** 두 개면 화면을 다 그릴 수 있다.
   같이 붙는 것: 포스트 등록창 버튼/규칙(Prime·부스트·앨범패스), 홈 "PASS" 표시.
2. **잔여 팝업/화면** — 신고·차단(채팅창 메뉴에 항목만 있음), 관심사/지역/소개 편집, 친구 오늘의 포스트(`GET /friends/:id/today-post` 서버도 미구현), 시간 게이트 화면 분기.
3. **다국어(한↔일) l10n**.
