...
> 📌 **여기까지가 다른 기기와 싱크된 범위다**(2026-08-03, `23f3195`).
> 다음 작업(다국어 ③단계)의 배경·설계·순서는 [09 인수인계](09-next-task-handoff.md)에 있다.
> 다음 "싱크용 문서" 요청은 **③단계 작업부터** 정리하면 된다.
...
2. **실시간 번역(사용자가 쓴 글)** — UI 다국어와 **완전히 별개**다. 껍데기만 있고 알맹이가 없다.
   ...
   - ⚠️ **무료 쿼터·패스 판정이 없다.** `GardenService.translate()`가 `userId`를 받지 않아 하루 2회 제한도, `TRANSLATE_PASS` 무제한도 동작하지 않는다. `daily_usage`의 `COMMENT_TRANSLATE`/`CHAT_TRANSLATE`는 만들어만 두고 미사용.
   - ⚠️ **BM 구멍**: 자동 번역 패스를 800루나에 팔면서 실제 혜택이 **아무것도 없다**. 지금은 패스스루라 드러나지 않지만, 번역 키를 붙이는 순간 **누구나 무제한**이 된다.
   - **키 없이 먼저 할 수 있는 것**: 쿼터 강제 + 패스 판정. 번역 결과와 무관하게 검증 가능하고 BM 구멍을 막는다. 공급자(DeepL/Google) 연동만 키 발급 후.
2. **앱 아이콘·스플래시 등 리소스** — [08 체크리스트](08-assets-checklist.md). 지금 Flutter 기본 아이콘 그대로다.
3. **외부 계정이 생기면**: 영수증 실검증(Google/Apple), 웹훅 서명 검증 + 구독 자동갱신, 광고 SDK, 푸시(FCM/APNs), S3 실연동, 서명 키스토어, iOS 빌드.
