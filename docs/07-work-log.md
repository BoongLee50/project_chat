...
## 1. 현재 상태 스냅샷 (2026-08-03 기준)

**한 줄 요약**: 기획서 화면 1~30 전부 구현. **전 화면이 로컬 서버·DB와 실제 연동 완료** — 로그인/온보딩 · 프로필 · 홈(오늘의 포스트) · 달빛가든 · 대화방/채팅창(WebSocket 실시간) · 친구. **서버 도메인 전부**(Flyway **V7**) 구현 완료. **UI 다국어(한↔일) ①·②단계 완료** — 화면 전량 + **오류 문구까지** ARB 이전(441키). **번역 무료 쿼터·패스 판정**도 붙어 `TRANSLATE_PASS`가 실제 혜택을 갖는다. 남은 건 **일본어 마감(③단계, [09](09-next-task-handoff.md))** · 번역 공급자 연동(API 키) · 리소스 · 외부 계정이 필요한 것들.
...
| DB | 로컬 MariaDB 11.4.5, Flyway **V7까지 적용**(V1 초기 · V2 posts 등록창/교체 · V3 post_comments · V4 chat · V5 friendships 양방향 · V6 BM · **V7 번역 쿼터**) |
...
| 다국어 | **①·②단계 완료** — `app_ko.arb`/`app_ja.arb` 각 **441키**(화면 378 + 오류 61 + 번역 2). 기기 언어를 따르고 미지원 언어는 한국어 폴백. 오류 문구는 서버 `ErrorCode`(56종)를 클라가 번역한다. 에뮬 `ja-JP` 실확인. 남은 건 **원어민 검수**(③단계) |

**남은 화면/기능 한눈에**: **기획서 화면은 전부 구현 완료**(화면 1~30). 남은 건 다국어 ③단계(일본어 마감) · 번역 공급자 연동 + 클라 번역 버튼 · 리소스(앱 아이콘·스플래시 등) · 외부 계정이 필요한 것들.
