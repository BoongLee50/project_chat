...
- [ ] **다국어 ②단계 — 오류 문구**: 서버 `ErrorCode` 10종 → 50여 종 세분화 + 클라 매핑.
  클라 프로바이더 계층 에러 9개(`BuildContext`가 없어 `L10n`을 못 쓰는 자리)도 같이 정리.
  👉 **실측 인벤토리·코드 이름 초안·진행 순서는 [09 인수인계](09-next-task-handoff.md)에 정리돼 있다**
- [ ] **다국어 ③단계** — 일본어 원어민 검수(현재 378키 전부 초벌), Noto Sans JP 폰트, (선택) 앱 내 언어 전환 UI
