...
> 기준 기획: **Plan_4(`수정2`)**. 🚨 **야간 운영시간 게이트는 폐지됐다 — 24시간 운영이다**(Plan_3 ①단계).
> 대신 **영업일 경계가 KST 18시**(`app.session.rollover-hour`)다. 아래 본문에 17시/06시가 남아 있으면
> **옛 Plan_2 잔재**이니 그대로 믿지 말 것.
> 📌 게이트는 사라졌지만 **원칙은 남는다** — 제한은 클라 화면이 아니라 **서버가 건다**.
> 시간·권한 제한이 필요한 새 API에는 반드시 서버측 검사를 함께 넣을 것. 상점/구독(BM)은 §1.8, 친구는 양방향(요청/수락).
> 상태: **설계 초안**. 서버 스택 **확정**(Spring Boot + MariaDB/MyBatis + Redis(선택 구성)). 채팅 보관정책은 **확정 — 방 타입과 무관하게 30일**, FIFO 삭제(2026-09-06에 "친구 1년"을 없앴다). 클라 "대화 삭제"는 UI만 — [02 스키마](02-db-schema.md) 참고. 🚨 **이 기간이 곧 대화방의 수명이다**(§1.6). 기획서 화면 기준으로 매핑.
...
> 🗑️ **`GET /system/gate`는 삭제됐다**(Plan_3 ①단계, 게이트 폐지). 서버에 그 경로가 없다.
> ⚠️ 그런데 **없는 경로도 404가 아니라 401을 준다** — 인증 인터셉터가 라우팅보다 먼저 돌기 때문이다.
> 그래서 옛 문서를 보고 이걸 부르면 "서버가 죽었나"로 오해하기 쉽다([07 §3 함정 #57](07-work-log.md)).
...
**카드의 두 제스처(2026-08-30 기획 확인)** — 헷갈리기 쉬워 여기 적어 둔다.
- **좌우 스와이프 = 사람을 넘긴다**(다음 카드).
- **카드를 눌렀다 떼면 = 그 사람의 사진 뷰어**가 뜨고, **그 안에서** 좌우 스와이프로 사진을 넘긴다.
  카드에는 **메인 한 장**만 보이고 우상단에 `1/N` 숫자 표기가 붙는다.
- 두 스와이프를 한 화면에 겹치면 어느 쪽인지 알 수 없어진다. 그래서 사진 넘기기를 창 안으로 뺐다.
- 탭은 **tap-up**(누르고 뗌)으로 받는다 — 손가락이 닿자마자 열면 스와이프하려던 손짓이 창을 연다.
...
- 🚨 **한 스레드는 두 사람의 1:1 대화다** — **글쓴이**와 **그 스레드를 시작한 사람**.
  답글은 **번갈아** 달린다: `A가 글 → B가 댓글 → A가 답글 → B가 답글`.
...
종료·차단·신고된 항목은 목록에서 제거된다(어느 **한쪽**이 나가기·신고·차단만 해도 양쪽에서 사라진다 — 기획 6-1).
~~`대화 종료` 마크 30분 후 삭제~~ → 게이트와 함께 폐지. 지금은 아래 30일 규칙이 방의 수명이다.
...
**신고·차단 부수효과(둘 다 동일)** — 2026-09-06 기획서 6-2로 범위가 넓어졌다:
1. 두 사람 사이의 **대기 중인 대화 신청을 양방향으로 `BLOCKED`** → 양쪽 [받은 신청]에서 사라진다
2. **친구 관계 즉시 해제**(요청 중이든 성립이든) + 상대에게 `FRIEND_STATE(removed)`
3. **살아 있는 대화방 종료**(타입 무관) + 양쪽에 `ROOM_STATE(ended)`
4. 🚨 **서로의 포스트가 상대 가든에서 사라진다 — 양방향이다.** A가 B를 차단하면 **B의 목록에서도 A가 안 뜬다.**
   진입 시 정해진 순서에 남아 있어도 페이지를 채울 때 다시 걸러진다(스크롤 중에도 즉시 사라진다).
5. `GET /users/{id}/post-info`도 **409 `TARGET_BLOCKED_OR_REPORTED`** 로 막힌다 —
   목록에서만 빼면 낡은 목록이 상세로 들어오는 통로가 된다.

이후 그 상대에게는 대화 신청·친구 요청도 막힌다. 신고 사실 자체는 상대에게 알리지 않는다.

**받은 신청은 답하지 않으면 사라진다**(기획 6-2): 대화 신청·친구 신청 모두 **14일 무응답 시 만료**.
대화 신청은 `status = EXPIRED`(V24), 친구 신청은 **행 삭제**. 배치는 매일 18:10
(`app.scheduler.request-expiry-cron`), 기간은 `app.chat.request-expire-days` / `app.friend.request-expire-days`.
🚨 **만료는 거절이 아니다** — `REJECTED`로 두면 거절에 딸린 *1일 재신청 금지*가 걸려,
내가 답을 안 했을 뿐인데 상대가 다시 신청도 못 하게 된다.
...
- **무료 5회는 계정당 평생**이다(⑦단계, `lifetime_usage` V21 — 초기화되지 않는다). 프라임이면 무제한.

친구는 **양방향(상호 동의)** — 요청→수락 시 친구가 되며 대화방이 만들어진다. 최대 친구 수는
`app.friend.max-count`(**100**, Plan_3에서 과금과 무관하게 통일).
⚠️ **"친구 방은 24시간 예외"는 폐지됐다**(2026-09-06) — 친구 방도 30일이면 닫히고, 필요하면
`POST /chat/rooms:with/{userId}`로 새로 만든다([02 §1.6](02-db-schema.md)).
...
> 🗑️ **`SYSTEM_CLOSE`는 삭제됐다**(2026-09-06). 06시 일괄 종료를 알리던 옵코드인데
> 게이트가 폐지되며 보낼 일이 없어졌고, 클라는 한 번도 처리한 적이 없다.
> 방이 닫히는 건 이제 **개별 방마다 `ROOM_STATE(ended)`** 로만 온다(30일 무대화, §1.6).

### 2.3 소켓 vs REST 요약
- 🔌 소켓: 채팅 송수신, 대화방 상태변화, 대화신청·친구요청 수신알림, 친구 관계 변화, 프레즌스, 미확인 카운트.
