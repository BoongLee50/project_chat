...
///
/// ---
/// 🚨 **여기 있는 [IconData]는 임시다**(기획 2026-09-19).
/// **관심사 화면의 버튼·아이콘은 전부 이미지 파일이 정본**이고, 코드가 그리거나
/// 텍스트로 대체하는 것이 아니다. 지금은 37종 중 **`영화` 한 장**(`Interest_movie.png`)만
/// 와 있어서 나머지를 `Icons.*`로 버티는 중이다.
///
/// 리소스가 오면 이 표를 **코드 → 에셋 경로**로 바꾸고 `ArtImage`로 그리면 된다.
/// ⚠️ 그때 [interestLabel]도 함께 걷힐 수 있다 — 받은 `Interest_movie.png`는
/// **아이콘과 "영화" 글자가 한 장에 같이 구워져 있다**(204×89). 즉 라벨이 그림에 들어 있으면
/// **일본어판이 따로 필요하다**(14 §2). 남은 것과 규격은 [08 §0-2](docs/08-assets-checklist.md).
...
abstract final class ProfileCatalog {
  ...
  ///
  /// ⚠️ **고르는 개수가 3일 뿐 종류는 아래 37종 그대로다.** 목록을 줄이는 것이 아니다.
  ///
  /// 📌 개수 표시에는 숫자를 박지 않았다 — ARB가 `{max}`로 받아 조립하므로
  /// 이 상수만 바꾸면 `(2/3)`·`저장하기 (2/3)`가 전부 따라온다.
